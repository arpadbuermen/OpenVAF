    ...
    pub fn as_constexprval(&self) -> Option<ConstExprValue> {
        ...
        match &*val.unwrap() {
            ast::Expr::Literal(lit) => match lit.kind() {
                ...
                LiteralKind::StdRealNumber(f) => Some(ConstExprValue::Float(if negate { (-f.value()).into() } else { f.value().into() })), 
                LiteralKind::SiRealNumber(f) => Some(ConstExprValue::Float(if negate { (-f.value()).into() } else { f.value().into() })), 
                LiteralKind::IntNumber(i) => Some(ConstExprValue::Int(if negate { -i.value() } else { i.value() })), 
                _ => None
            }, 
            _ => None
        }
