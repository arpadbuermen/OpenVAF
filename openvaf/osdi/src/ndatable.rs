...
use hir_def::ndatable::{AttrEntry, DisciplineEntry, NDANatureIdResolution, NDATable, NatureEntry};
use lasso::Rodeo;
use std::vec::Vec;
...
fn osdi_nature(nature: &NatureEntry, literals: &mut Rodeo) -> OsdiNature {
    ...
    OsdiNature {
        ...
        attr_start: if nature.attr_range.is_empty() { u32::MAX } else { i1 },
        num_attr: if nature.attr_range.is_empty() { 0 } else { i2 - i1 },
    }
}
...
fn osdi_discipline(discipline: &DisciplineEntry, literals: &mut Rodeo) -> OsdiDiscipline {
    ...
    OsdiDiscipline {
        ...
        attr_start: if discipline.attr_range.is_empty() { u32::MAX } else { i1 },
        num_attr: if discipline.attr_range.is_empty() { 0 } else { i2 - i1 },
    }
}
