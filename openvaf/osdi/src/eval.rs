...
impl<'ll> InlineCallbackBuilder<'ll> for AbortCallback {
    fn build_inline(
        ...
        unsafe {
            ...
            // Get the raw pointer to llbuilder from the Builder struct
            let builder_ptr = builder as *const Builder<'_, '_, 'll>;
            let llbuilder =
                std::ptr::addr_of!((*builder_ptr).llbuilder) as *mut llvm_sys::LLVMBuilder;

            // Branch always to return block
            let cond = cx.const_bool(true);
            LLVMBuildCondBr(
                llbuilder, // Pass the raw pointer directly
            ...
            LLVMBuildRet(llbuilder, NonNull::from(ret_flags).as_ptr());

            // Position builder at start of continue block
