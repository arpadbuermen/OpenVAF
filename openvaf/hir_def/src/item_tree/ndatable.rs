...
use super::{Discipline, DisciplineAttrKind, ItemTree, Nature, NatureRef, NatureRefKind};
...
impl NDACtx {
    ...
    fn create_nature(&self, nature: &Nature, table: &mut NDATable) -> NatureEntry {
        ...
        for ndx in nature.attrs.clone() {
            ...
            if let Some(value) = &attr.value {
                table
                    .attributes
                    .push(AttrEntry { name: attr.name.to_string(), value: value.clone() });
            }
        }
    ...
    fn create_derived_nature(
        &self,
        discipline: &Discipline,
        is_flow: bool,
        name: String,
        parent_ndx: &Idx<NatureEntry>,
        table: &mut NDATable,
    ) -> NatureEntry {
        let i1 = table.attributes.next_key();
    ...
    pub fn create_discipline(
        &self,
        discipline: &Discipline,
        flow_index: Option<Idx<NatureEntry>>,
        potential_index: Option<Idx<NatureEntry>>,
        table: &mut NDATable,
    ) -> DisciplineEntry {
        let i1 = table.attributes.next_key();
        for ndx in discipline.extra_attrs.clone() {
            ...
            if let Some(value) = &attr.value {
                table.attributes.push(AttrEntry { name, value: value.clone() });
            }
        }
        let i2 = table.attributes.next_key();
        DisciplineEntry {
            name: discipline.name.to_string(),
            flow_nature: if let Some(flow_index) = flow_index {
                NDANatureIdResolution::NDAId(flow_index)
            } else {
                NDANatureIdResolution::Unresolved
            },
            potential_nature: if let Some(potential_index) = potential_index {
                NDANatureIdResolution::NDAId(potential_index)
            } else {
                NDANatureIdResolution::Unresolved
            },
            attr_range: IdxRange::new(i1..i2),
    ...
    pub fn resolve_nature_reference(
        &self,
        table: &NDATable,
        natref: &NatureRef,
    ) -> NDANatureIdResolution {
        if natref.kind == NatureRefKind::DisciplineFlow
            || natref.kind == NatureRefKind::DisciplinePotential
        {
            // Lookup discipline
            ...
            for tree_nature in self.tree.data.natures.iter() {
                ...
                if let Some(idx) = table.nature_name_map.get(&tree_nature.name.to_string()) {
                    ...
                    if table.natures[*idx].parent == NDANatureIdResolution::Unresolved {
                        table.natures[*idx].parent =
                            if let Some(natref) = tree_nature.parent.as_ref() {
                    ...
                    if table.natures[*idx].ddt_nature == NDANatureIdResolution::Unresolved {
                        table.natures[*idx].ddt_nature =
                            if let Some((natref, _)) = tree_nature.ddt_nature.as_ref() {
                                let natres = self.resolve_nature_reference(table, natref);
                                if natres != NDANatureIdResolution::Unresolved {
                                    changed = true;
                                }
                    ...
                    if table.natures[*idx].idt_nature == NDANatureIdResolution::Unresolved {
                        table.natures[*idx].idt_nature =
                            if let Some((natref, _)) = tree_nature.idt_nature.as_ref() {
                                let natres = self.resolve_nature_reference(table, natref);
                                if natres != NDANatureIdResolution::Unresolved {
                                    changed = true;
                                }
            ...
            for tree_discipline in self.tree.data.disciplines.iter() {
                ...
                if let Some(idx) = table.discipline_name_map.get(&tree_discipline.name.to_string())
                {
                    ...
                    if table.disciplines[*idx].flow_nature == NDANatureIdResolution::Unresolved {
                        table.disciplines[*idx].flow_nature =
                            if let Some((natref, _)) = tree_discipline.flow.as_ref() {
                                let natres = self.resolve_nature_reference(table, &natref);
                                if natres != NDANatureIdResolution::Unresolved {
                                    changed = true;
                                }
                    ...
                    {
                        table.disciplines[*idx].potential_nature =
                            if let Some((natref, _)) = tree_discipline.potential.as_ref() {
                                let natres = self.resolve_nature_reference(table, &natref);
                                if natres != NDANatureIdResolution::Unresolved {
                                    changed = true;
                                }
            ...
            if let Some((nature_ref, _)) = &discipline.flow {
                ...
                if let Some(parent_ndx) = maybe_parent {
                    ...
                    if flow_attr_set {
                        ...
                        let nat = self.create_derived_nature(
                            discipline,
                            true,
                            flow_name.clone(),
                            &parent_ndx,
